...
import { createSequenceAction } from '@/app/admin/actions'

const DEPARTMENTS = ['Product', 'Design', 'Content', 'Animation']

// Known deliverable types, from real historical data. Admin will be able to
// add/edit this list from Settings in a future update — for now it's fixed here.
const KNOWN_TYPES = [
...
export function NewSequenceForm({ claimedTypes }: { claimedTypes: string[] }) {
  const [stages, setStages] = useState<StageRow[]>([
    { name: 'Content', department: 'Content' },
  ...
  async function handleSubmit(formData: FormData) {
    const cleanStages = stages
      .map(s => ({ name: s.name.trim(), department: s.department }))
      .filter(s => s.name)
    ...
    setSubmitting(true)
    try {
      await createSequenceAction(formData)
      setStages([{ name: 'Content', department: 'Content' }, { name: 'Design', department: 'Design' }])
      setSelectedTypes([])
    } catch (err) {
      console.error(err)
  ...
  return (
    <form action={handleSubmit} className="card">
      <div className="panel-head"><h2>New sequence</h2></div>
      <div className="panel-body">
        <div className="form-row">
          <label>Name</label>
          <input name="name" placeholder="e.g. Infographic — Fast Track" required />
        </div>
        <div className="form-row">
          <label>Description</label>
          <input name="description" placeholder="When to use this" />
        </div>
          ...
          >
            {selectedTypes.map(type => (
              <span
                key={type}
                style={{
                  display: 'inline-flex', alignItems: 'center', gap: 5, background: 'var(--primary-soft)',
                  color: 'var(--primary)', borderRadius: 5, padding: '3px 8px 3px 10px', fontSize: 12, fontWeight: 600,
                }}
              >
                {type}
                <button
            ...
            {selectedTypes.length === 0 && (
              <span style={{ color: 'var(--muted)', fontSize: 13 }}>Select deliverable types…</span>
            )}
          ...
          {typesOpen && (
            <div
              style={{
                ...
                maxHeight: 220, overflowY: 'auto', boxShadow: '0 6px 18px rgba(28,37,33,0.12)',
              }}
            >
              {KNOWN_TYPES.filter(t => !selectedTypes.includes(t)).map(type => {
                ...
                return (
                  <div
                    ...
                    style={{
                      padding: '9px 12px', cursor: claimed ? 'not-allowed' : 'pointer', fontSize: 13,
                      color: claimed ? 'var(--muted)' : 'var(--ink-soft)',
              ...
              {KNOWN_TYPES.filter(t => !selectedTypes.includes(t)).length === 0 && (
                <div style={{ padding: '9px 12px', fontSize: 12.5, color: 'var(--muted)' }}>Nothing left to add.</div>
              )}
            </div>
          )}
        </div>
